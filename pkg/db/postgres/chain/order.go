...
func (db *DB) initOrders(ctx context.Context) error {
	...
	query := fmt.Sprintf(`
		...
		CREATE INDEX IF NOT EXISTS idx_orders_height ON %s(height);
		CREATE INDEX IF NOT EXISTS idx_orders_status ON %s(status);
		CREATE INDEX IF NOT EXISTS idx_orders_committee ON %s(committee);
	`, ordersTable, ordersTable, ordersTable, ordersTable)
